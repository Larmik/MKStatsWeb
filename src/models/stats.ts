...
export class Stats {
    ...
    mapsWon?: string
    shockCount?: number
    highestPlayerScore?: ({first: number, second: string})
    lowestPlayerScore?: ({first: number, second: string})

    constructor(warStats: WarStats,
        mostPlayedTeam: TeamStats| undefined,
        mostDefeatedTeam: TeamStats| undefined ,
        lessDefeatedTeam: TeamStats| undefined,
        warScores: WarScore[],
        maps: TrackStats[],
        averageForMaps: TrackStats[]
    ) {
        ...
        this.bestMap = averageForMaps.sort((a,b) => (a.teamScore ?? 0) > (b.teamScore ?? 0) ? -1 : 1)[0]
        this.worstMap = averageForMaps.sort((a,b) => (a.teamScore ?? 0) > (b.teamScore ?? 0) ? 1 : -1)[0]
        this.bestPlayerMap = averageForMaps.sort((a,b) => (a.playerScore ?? 0) > (b.playerScore ?? 0) ? -1 : 1)[0]
        this.worstPlayerMap = averageForMaps.sort((a,b) => (a.playerScore ?? 0) > (b.playerScore ?? 0) ? 1 : -1)[0]
        this.mostPlayedMap = averageForMaps.sort((a,b) => (a.totalPlayed ?? 0) > (b.totalPlayed ?? 0) ? -1 : 1)[0]
    ...
    public getPlayerScores(playerId: string, players: Player[]) {
        let finalList: ({first: number, second: string})[] = []
        this.warStats.wars.forEach(war => {
            ...
            .map(pos => ({first: pos.first, second: pos.second.map(pos => NumberUtils.positionToPoints(pos)).reduce((a,b) => a+b)}))
            .filter(value => value.first.player_id.toString() == playerId)
            .map(value => ({ first: value.second, second: war.createdDate}))
            .forEach(item => {
                finalList.push(item)
            })
        })
